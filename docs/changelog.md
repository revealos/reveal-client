...
**First Pilot Release** - The Reveal SDK is now ready for production use with your first customer pilot.

### What's Included

**Core Functionality**
- Automatic friction detection (stall, rage click, backtrack, no progress)
- Real-time contextual nudge delivery based on backend decisions
- Event tracking and analytics with automatic batching and retry logic
- Framework-agnostic core with optional React hooks for seamless integration

**Security & Privacy**
- Comprehensive PII scrubbing (30+ patterns) to protect user data
- HTTPS enforcement for all API endpoints (with localhost exception for development)
- Email redaction in URLs and event payloads
- Structured audit logging for compliance and debugging

**Developer Experience**
- Full TypeScript support with exported types
- React hooks (`useNudgeDecision`) for easy integration
- Framework-agnostic support (works with Vue, Angular, Svelte, or vanilla JS)
- Comprehensive documentation and integration guides
- Production-ready error handling that never crashes your app

**Production Readiness**
- Robust error handling and crash prevention
- Automatic event batching and retry logic
- Environment-aware timeouts (production vs development)
- DOM isolation to prevent conflicts with your app
- Comprehensive test coverage

### Added

- **Core API**: `Reveal.init()`, `Reveal.track()`, `Reveal.onNudgeDecision()`
- **TypeScript type definitions**: Full type support exported from `@reveal/client`
...
- **useNudgeDecision() React Hook**: React hook that subscribes to nudge decisions and provides UI-ready decision state with tracking handlers
  - Automatically subscribes to `Reveal.onNudgeDecision` on mount
...
- **Comprehensive documentation**: README with quick start guide, API documentation, Architecture documentation, Security considerations, SOC2 compliance notes

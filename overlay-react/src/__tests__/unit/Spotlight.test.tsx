/**
 * Spotlight Component Tests
 *
 * These tests verify the React ↔ Web Component INTEGRATION CONTRACT:
 * 1. Spotlight renders <reveal-spotlight> custom element
 ...
 * - Spotlight-specific behavior (tested in @reveal/overlay-wc)
 */
...
describe("Spotlight - React/WC Integration Contract", () => {
  ...
  beforeEach(() => {
    ...
    if (!customElements.get("reveal-spotlight")) {
      class RevealSpotlightStub extends HTMLElement {
        _decision: any = null;
      ...
      customElements.define("reveal-spotlight", RevealSpotlightStub);
    }
  });
  ...
  describe("Element Rendering", () => {
    it("renders reveal-spotlight custom element", () => {
      const decision = createMockDecision();
      render(<Spotlight decision={decision} onDismiss={mockOnDismiss} />);

      const wcElement = document.querySelector("reveal-spotlight");
  ...
  describe("Decision Property Passing", () => {
    it("sets decision property on WC element", async () => {
      const decision = createMockDecision({
        id: "test-123",
        body: "Test spotlight message",
      });
      render(<Spotlight decision={decision} onDismiss={mockOnDismiss} />);

      await waitFor(() => {
        const wcElement = document.querySelector("reveal-spotlight") as any;
        expect(wcElement).toBeTruthy();
        expect(wcElement.decision).toEqual(
    ...
    it("updates decision property when prop changes", async () => {
      ...
      const { rerender } = render(
        <Spotlight decision={decision1} onDismiss={mockOnDismiss} />
      );
      ...
      rerender(<Spotlight decision={decision2} onDismiss={mockOnDismiss} />);
  ...
  describe("CustomEvent Handling", () => {
    ...
    it("calls onTrack when reveal:shown event is fired", async () => {
      ...
      await waitFor(() => {
        ...
        expect(mockOnTrack).toHaveBeenCalledWith("nudge", "nudge_shown", {
          nudgeId: "nudge-789",
        });
      });
    });
    ...
    it("handles events fired before listeners attached", async () => {
      ...
      const { container } = render(
        <Spotlight decision={decision} onDismiss={mockOnDismiss} />
      );

      // Fire event immediately (before listeners might be attached)
      const wcElement = container.querySelector("reveal-spotlight")!;
      wcElement.dispatchEvent(
        new CustomEvent("reveal:dismiss", {
